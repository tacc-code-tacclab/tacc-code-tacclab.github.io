...
for _=1,150 do g:update(1/60) end
...
for _,level in ipairs({6,10}) do
    ...
    local arrival=nil
    for _=1,4000 do balanced:update(1/480); if balanced.poison[target]>0 then arrival=balanced.time; break end end
    assert(arrival and arrival>(#path-1)/Core.SPEED)
end

-- Later gardens add ants that chase, hurt and carve connected passages.
local first,middle,last=Core.new(1),Core.new(6),Core.new(10)
assert(first.difficulty.antCount==0 and middle.difficulty.antCount==3 and last.difficulty.antCount==5)
assert(last.difficulty.interval<middle.difficulty.interval and middle.difficulty.interval<first.difficulty.interval)
assert(last.difficulty.warning<first.difficulty.warning and last.difficulty.floodStep<first.difficulty.floodStep)
assert(last.difficulty.farmerSpeed>first.difficulty.farmerSpeed and last.difficulty.antSpeed>middle.difficulty.antSpeed)
...
print('Lua core: 10 gardens, directional fading poison, instant death, fast farmer, digging ants and retry passed.')
