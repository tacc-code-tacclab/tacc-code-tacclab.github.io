...
test('horizontal and bending tunnels let the mole outrun poison from level 6 onward',()=>{
 ...
 for(const level of [6,10]){
  ...
  let arrival=null;for(let i=0;i<4000&&!arrival;i++){g.update(1/480);if(g.poison[target]>0)arrival=g.time;}
  assert(arrival>(path.length-1)/SPEED,`level ${level}: zig-zag poison ${arrival}s must trail the mole`);
 }
});
