...
(function (root, factory) {
  ...
  class Game {
    ...
    carryPoisonIntoAntTunnel(k,c,r) {
      ...
      if(wave){
        const life=Math.max(0,this.difficulty.poisonLife-wave.age);if(life<=0)return;
        this.poison[k]=Math.max(this.poison[k],life);this.injectWaveCell(wave,k,c-pc,r-pr);
      } else {
        this.poison[k]=Math.max(this.poison[k],this.difficulty.poisonLife);this.waves.push(this.makeWave(k));
      }
    ...
    step(dt,dx,dy) {
      ...
      for(const wave of this.waves) {
        wave.age+=dt;
        if(wave.age>=this.difficulty.poisonLife)continue;
        const ready=[],waiting=[];
        ...
        for(const entry of ready) {
          ...
          const life=Math.max(0,this.difficulty.poisonLife-wave.age);
          this.poison[k]=Math.max(this.poison[k],life);
      ...
      this.waves=this.waves.filter(w=>w.age<this.difficulty.poisonLife&&w.pending.length);
      if(this.poisonTouchesPlayer()) return;
      this.updateAnts(dt);
