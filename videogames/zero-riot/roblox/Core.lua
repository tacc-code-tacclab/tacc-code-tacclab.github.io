-- Bubble Riot v3: deterministic arithmetic bubble shooter. Standard Lua / Luau.
...
function R.valueOf(g,v,ammo) if g.config.op=='+' then return v+ammo else return v*ammo end end
function R.required(g,v) if g.config.op=='+' then return g.config.target-v else return g.config.target/v end end
...
function R.create(seed,level,carry)
 level=clamp(math.floor(level or 1),1,#R.LEVELS);carry=carry or {};local config=R.LEVELS[level];local random=R.rng(R.seedNumber(tostring(seed)..'/B'..level));local bank={score=carry.score or 0,totalStars=carry.totalStars or 0}
 local g={seed=tostring(seed),level=level,config=config,random=random,bank=bank,score=bank.score,totalStars=bank.totalStars,stars=0,time=0,shots=0,misses=0,combo=0,power=0,rainbowReady=false,selected=0,ammo={},angle=0,projectile=nil,cooldown=0,ended=false,won=false,campaignComplete=false,bubbles={},bumpers={},events={},nextId=0}
 local values={};for v=(config.op=='+' and 1 or 2),config.target-1 do if config.op=='+' or config.target%v==0 then values[#values+1]=v end end
...
function R.resolve(g,id,shot)
 local b=find(g,id);if not b then return end;local result=R.valueOf(g,b.v,shot.value)
 if not shot.rainbow and result~=g.config.target then miss(g,b.v..' '..g.config.op..' '..shot.value..' = '..result..' · aim for '..g.config.target,b.x,b.y);return end
 ...
 local points=#popped*100+#dropped*150+g.combo*50;g.score=g.score+points
 g.events[#g.events+1]={type='pop',popped=popped,dropped=dropped,x=b.x,y=b.y,points=points,message=shot.rainbow and 'RAINBOW RIOT!' or (b.v..' '..g.config.op..' '..shot.value..' = '..g.config.target),combo=g.combo}
