...
(function(){
  ...
  let best=0;try{best=Number(localStorage.getItem('zero-riot-best-v1'))||0;}catch(_){}
  ...
  if(urlSeed)$('start').innerHTML='PLAY SHARED ARENA <span>↗</span>';
  ...
  function clearInput(){keys.clear();stick.x=0;stick.y=0;stick.id=null;drag=null;$('knob').style.transform='translate(0,0)';}
  function show(id,visible){$(id).classList.toggle('hidden',!visible);}
  function start(seedValue){g=R.create(seedValue);mode='play';accumulator=0;particles=[];rings=[];floats=[];shake=0;clearInput();show('menu',false);show('paused',false);show('results',false);$('pause').disabled=false;$('flip').disabled=false;$('footer-seed').textContent=g.seed;$('share-note').textContent='';show('share-fallback',false);audio();canvas.focus({preventScroll:true});updateHUD();}
  ...
  $('flip').addEventListener('click',doFlip);$('pause').addEventListener('click',pause);$('resume').addEventListener('click',pause);$('restart').addEventListener('click',()=>start(g.seed));$('replay').addEventListener('click',()=>start(g.seed));
  ...
  canvas.addEventListener('pointerdown',e=>{if(mode!=='play')return;drag={...point(e),id:e.pointerId};canvas.setPointerCapture(e.pointerId);e.preventDefault();});canvas.addEventListener('pointermove',e=>{if(drag&&drag.id===e.pointerId)drag={...point(e),id:e.pointerId};});for(const name of ['pointerup','pointercancel','lostpointercapture'])canvas.addEventListener(name,e=>{if(drag&&drag.id===e.pointerId)drag=null;});
  const joystick=$('joystick');function stickMove(e){const r=joystick.getBoundingClientRect();let x=(e.clientX-r.left-r.width/2)/(r.width*.32),y=(e.clientY-r.top-r.height/2)/(r.height*.32),len=Math.hypot(x,y);if(len>1){x/=len;y/=len;}stick.x=x;stick.y=y;$('knob').style.transform=`translate(${x*30}px,${y*30}px)`;}
  joystick.addEventListener('pointerdown',e=>{if(mode!=='play')return;stick.id=e.pointerId;joystick.setPointerCapture(e.pointerId);stickMove(e);e.preventDefault();});joystick.addEventListener('pointermove',e=>{if(stick.id===e.pointerId)stickMove(e);});for(const name of ['pointerup','pointercancel','lostpointercapture'])joystick.addEventListener(name,e=>{if(stick.id===e.pointerId){stick.x=0;stick.y=0;stick.id=null;$('knob').style.transform='translate(0,0)';}});
  function input(){let x=(keys.has('d')||keys.has('arrowright')?1:0)-(keys.has('a')||keys.has('arrowleft')?1:0),y=(keys.has('s')||keys.has('arrowdown')?1:0)-(keys.has('w')||keys.has('arrowup')?1:0);if(stick.id!==null){x=stick.x;y=stick.y;}else if(drag){x=(drag.x-g.x)/18;y=(drag.y-g.y)/18;if(Math.hypot(x,y)<.12){x=0;y=0;}}return{x,y};}
  ...
  function events(){for(const e of g.events){
    ...
    if(e.type==='nova'){rings.push({...e,life:.6,max:.6,color:C.yellow});burst(e.x,e.y,C.yellow,48);floats.push({x:e.x,y:e.y-52,text:'ZERO! +'+e.points,color:C.yellow,life:1.25});shake=reduced?0:5;tone(110,.38,'triangle',.13,5);}
    ...
    if(e.type==='hurt'){floats.push({x:e.x,y:e.y-50,text:e.reason,color:C.red,life:1.2});burst(e.x,e.y,C.red,25);shake=reduced?0:7;tone(130,.25,'sawtooth',.06,.3);}
    if(e.type==='blocked')floats.push({x:e.x,y:e.y-50,text:'SHIELDED',color:C.cyan,life:.8});
    if(e.type==='end')finish();
  }g.events.length=0;}
  function finish(){mode='end';clearInput();$('pause').disabled=true;$('flip').disabled=true;const previous=best;if(g.score>best){best=g.score;try{localStorage.setItem('zero-riot-best-v1',String(best));}catch(_){}$('best').textContent=best.toLocaleString();}
    $('result-status').textContent=g.won?'90 SECONDS · REACTOR SURVIVED':'THREE HITS · REACTOR DOWN';$('result-score').textContent=g.score.toLocaleString();$('result-rank').textContent=g.score>previous?'NEW LOCAL BEST':g.novas>=12?'ZERO HERO':g.novas>=6?'CHAIN REACTION':g.novas>0?'GETTING THE HANG OF NOTHING':'NEXT TIME, CHASE ZERO';
    $('result-stats').innerHTML=`<span><b>${g.novas}</b>NOVAS</span><span><b>${g.bestChain}</b>LONGEST CHAIN</span><span><b>${g.kills}</b>HUNTERS BLASTED</span><span><b>${Math.floor(g.time)}s</b>SURVIVED</span>`;show('results',true);$('replay').focus({preventScroll:true});tone(g.won?660:150,.4,'triangle',.07,g.won?1.5:.5);
  }
  function updateHUD(){
    $('score').textContent=g.score.toLocaleString();$('charge').textContent=signed(g.charge);$('charge').style.color=g.charge===0?C.yellow:g.charge>0?C.cyan:C.pink;$('time').innerHTML=Math.ceil(90-g.time)+'<span>s</span>';$('lives').textContent='●'.repeat(g.lives)+'○'.repeat(3-g.lives);$('lives').setAttribute('aria-label',g.lives+' shields');
    $('phase').textContent=g.time<30?'01 / CHARGE UP':g.time<60?'02 / CROSS FIRE':'03 / ZERO HOUR';$('chain').textContent=g.chain?g.chain+' PICKUPS · POWER '+g.mass:'BUILD A CHAIN';$('danger').classList.toggle('warn',Math.abs(g.charge)>=7);$('danger').textContent=Math.abs(g.charge)>=7?'NEAR OVERLOAD':'LIMIT ±9';
    $('hint').textContent=g.charge===0?'Collect a charge to get started.':'Find '+signed(-g.charge)+' — or flip '+signed(g.charge)+'.';$('flip-label').textContent=g.flipCooldown>0?'RECHARGING '+g.flipCooldown.toFixed(1)+'s':'FLIP THE FIELD';$('recharge').style.transform=`scaleX(${1-g.flipCooldown/2})`;$('flip').disabled=mode!=='play'||g.flipCooldown>0;
  }
  ...
  function draw(){
    ...
    if(drag&&mode==='play'){ctx.strokeStyle='#a5afcc55';ctx.lineWidth=1;circle(drag.x,drag.y,10);ctx.stroke();}
    for(const p of g.pickups){if(p.wait>.3)continue;ctx.save();ctx.globalAlpha=p.wait>0?.45:1;const useful=g.charge!==0&&p.v===-g.charge,danger=Math.abs(g.charge+p.v)>9,color=p.v>0?C.cyan:C.pink;
    ...
    ctx.save();const pc=g.charge===0?C.yellow:g.charge>0?C.cyan:C.pink;ctx.globalAlpha=g.invulnerable>0&&mode==='play'?.65+.35*Math.sin(ambient*20):1;ctx.shadowBlur=reduced?0:24;ctx.shadowColor=pc;ctx.fillStyle='#131e36';ctx.strokeStyle=pc;ctx.lineWidth=4;circle(g.x,g.y,25);ctx.fill();ctx.stroke();ctx.shadowBlur=0;
    ...
    ctx.fillStyle=C.white;ctx.font='900 29px Arial';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(signed(g.charge),g.x,g.y+1);ctx.restore();
