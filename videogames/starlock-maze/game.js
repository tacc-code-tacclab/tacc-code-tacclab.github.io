(()=>{
  ...
  function initAudio(){if(audioCtx)return;try{audioCtx=new(window.AudioContext||window.webkitAudioContext)();master=audioCtx.createGain();master.gain.value=soundOn ? MASTER_VOLUME : 0;master.connect(audioCtx.destination)}catch{}}
  function tone(kind){
    if(!audioCtx||!soundOn)return;const now=audioCtx.currentTime,osc=audioCtx.createOscillator(),gain=audioCtx.createGain(),cfg={dot:[520,760,.035,"sine",.05],power:[190,1120,.38,"sawtooth",.13],warning:[560,185,.44,"square",.14],watch:[185,62,.72,"sawtooth",.24],safe:[240,620,.22,"triangle",.08],shoot:[890,250,.11,"square",.08],wall:[130,45,.18,"sawtooth",.15],enemy:[230,980,.25,"triangle",.12],hurt:[105,42,.45,"sawtooth",.2],level:[330,1320,.58,"triangle",.13],start:[180,760,.38,"triangle",.11]}[kind]||[260,420,.12,"sine",.08];
  ...
  function sleepDuration(level,fresh=false){const stage=Math.max(1,level),base=Math.max(3.15,9.6-(stage-1)*.72),variation=Math.max(1.3,3-(stage-1)*.13);return base+Math.random()*variation+(fresh?1.2:0)}
  function warningDuration(level){return Math.max(.72,1.45-(Math.max(1,level)-1)*.055)}
  function watchDuration(level){return 1.25+Math.min(.72,(Math.max(1,level)-1)*.06)+Math.random()*.55}
  ...
  function prepareLevel(level){
    ...
    state.mode="playing";setLight("sleep",sleepDuration(level,true),false);heldDirections.length=0;firing=false;buildMazeLayer();show(ui.clear,false);show(ui.pause,false);show(ui.gameover,false);updateHud();toast("LEVEL "+level+" · MOVE WHILE IT SLEEPS")
  }
  ...
  function setLight(mode,duration,audible=true){
    ...
    ui.app.classList.remove("alarm-warning","alarm-watch");if(mode==="warning")ui.app.classList.add("alarm-warning");else if(mode==="watch")ui.app.classList.add("alarm-watch");
    if(mode==="sleep"){ui.watch.querySelector("span").textContent="SAFE TO MOVE";if(audible)tone("safe")}
    else if(mode==="warning"){ui.watch.querySelector("span").textContent="STAR IS WAKING...";if(audible)tone("warning");toast("RED ALERT · RELEASE TO FREEZE")}
  ...
  function updateWatcher(dt){
    state.lightTimer-=dt;if(state.lightTimer<=0){if(state.lightMode==="sleep")setLight("warning",warningDuration(state.level));else if(state.lightMode==="warning")setLight("watch",watchDuration(state.level));else setLight("sleep",sleepDuration(state.level))}
  ...
  function menu(){if(state)updateRecords();state=null;heldDirections.length=0;firing=false;ui.app.classList.remove("alarm-warning","alarm-watch");show(ui.pause,false);show(ui.clear,false);show(ui.gameover,false);show(ui.hud,false);show(ui.watch,false);ui.hud.setAttribute("aria-hidden","true");ui.mobile.classList.remove("active");ui.fire.classList.remove("active");show(ui.menu,true);updateRecords()}
  ...
  function drawStarfish(x,y,radius,mode){
    ...
    ctx.translate(x,y);ctx.rotate(mode==="sleep"?Math.sin(ambient*1.7)*.09:Math.sin(ambient*8)*.025);const warningHot=Math.sin(ambient*24)>-.1,color=mode==="sleep"?"#7663e8":mode==="warning"?(warningHot?"#ff174f":"#ff8b36"):"#ff003c";ctx.shadowColor=color;ctx.shadowBlur=mode==="watch"?44:mode==="warning"?32:21;const gradient=ctx.createRadialGradient(-radius*.2,-radius*.25,1,0,0,radius);gradient.addColorStop(0,"#fff1c9");gradient.addColorStop(.18,mode==="sleep"?"#6beaff":mode==="warning"?"#ff665f":"#ff5c72");gradient.addColorStop(.55,color);gradient.addColorStop(1,mode==="sleep"?"#4b1e73":"#5d001e");ctx.fillStyle=gradient;ctx.strokeStyle=mode==="watch"?"#ffffff":mode==="warning"?"#ffd4d9":"#bfffff";ctx.lineWidth=Math.max(1.5,radius*.075);polygon(ctx,0,0,radius,5,-Math.PI/2,.42);ctx.fill();ctx.stroke();
  ...
  function drawAlarmOverlay(){if(state.lightMode!=="warning"&&state.lightMode!=="watch")return;const warning=state.lightMode==="warning",pulse=(Math.sin(ambient*(warning ? 24 : 7))+1)/2,on=Math.sin(ambient*24)>-.1;ctx.save();ctx.setTransform(dpr,0,0,dpr,0,0);ctx.globalCompositeOperation="screen";ctx.fillStyle="#ff003c";ctx.globalAlpha=warning ? (on ? .38 : .06) : .25+pulse*.08;ctx.fillRect(0,0,vw,vh);ctx.globalCompositeOperation="source-over";const vignette=ctx.createRadialGradient(vw*.5,vh*.46,Math.min(vw,vh)*.1,vw*.5,vh*.46,Math.max(vw,vh)*.7);vignette.addColorStop(0,"rgba(255,0,55,0)");vignette.addColorStop(1,"rgba(255,0,45,.94)");ctx.fillStyle=vignette;ctx.globalAlpha=warning ? (on ? .92 : .18) : .78;ctx.fillRect(0,0,vw,vh);ctx.globalAlpha=warning ? (on ? 1 : .25) : .86;ctx.strokeStyle="#ff174f";ctx.lineWidth=Math.max(8,Math.min(vw,vh)*.018);ctx.strokeRect(0,0,vw,vh);ctx.restore()}
  function drawGame(){drawBackdrop(ambient);ctx.save();if(state.shake>0)ctx.translate((Math.random()-.5)*state.shake,(Math.random()-.5)*state.shake);drawMaze();drawPellets();for(const enemy of state.enemies)drawEnemy(enemy);drawPlayer(state.player);drawBullets();drawParticles();const starX=originX+mapW*.8,starY=originY-(coarse?36:49),starRadius=Math.min(coarse?48:64,tile*(coarse?2.12:2.25));drawStarfish(starX,starY,starRadius,state.lightMode);ctx.restore();if(state.flash>0){ctx.globalAlpha=state.flash*.25;ctx.fillStyle=state.lightMode==="watch"?"#ff365f":"#62f6ff";ctx.fillRect(0,0,vw,vh);ctx.globalAlpha=1}drawAlarmOverlay()}
  ...
  ui.play.addEventListener("click",createRun);ui.pauseButton.addEventListener("click",pause);ui.resume.addEventListener("click",resume);ui.quit.addEventListener("click",menu);ui.next.addEventListener("click",nextLevel);ui.again.addEventListener("click",createRun);ui.home.addEventListener("click",menu);ui.sound.addEventListener("click",()=>{soundOn=!soundOn;safeStorage.set("starlockSound",soundOn);if(master)master.gain.value=soundOn ? MASTER_VOLUME : 0;if(soundOn){initAudio();audioCtx?.resume();tone("safe")}updateHud()});

  window.__STARLOCK_DIAGNOSTICS__=()=>state?{mode:state.mode,score:state.score,level:state.level,lives:state.lives,lightMode:state.lightMode,lightTimer:state.lightTimer,lightTotal:state.lightTotal,power:state.power,ammo:state.ammo,remaining:state.pellets.size+state.bonuses.size,wallsBlasted:state.wallsBlasted,player:{x:state.player.x,y:state.player.y,dir:state.player.dir,wanted:state.player.wanted,facing:state.player.facing,moved:state.player.moved,open:["left","right","up","down"].filter(name=>canMove(state.player,name))},enemies:state.enemies.map(enemy=>({x:enemy.x,y:enemy.y,dir:enemy.dir,spawnDelay:enemy.spawnDelay})),bullets:state.bullets.length,held:[...heldDirections],firing,simHz:Math.round(1/SIM_STEP)}:{mode:"menu",best,bestLevel};
