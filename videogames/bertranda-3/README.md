# BERTRANDA 3 — Endless Realms

Standalone first-person horror shooter for PC and phone, forked from Bertranda 2. Both previous editions remain available unchanged.

## Faster controls

Walk at 5.5 metres/second (previously 3.7) and sprint at 8.4 (previously 5.8), with the same stamina system. Default mouse and touch look speeds are about 50% higher. Choose Precise, Fast (default), or Very fast in the title or pause menu; changing it in Pause takes effect when resuming, without restarting.

Camera input is applied before movement, so a turn and a step use the same heading. Quick touch swipes retain their full travel instead of being cut to 100 pixels per event. Both mouse and touch shots use the current camera matrix. Movement keeps bounded simulation steps, wall collision and diagonal normalization. The minimap, golden bullet mission, organ score and creature behaviour are inherited from Bertranda 2.
...
Every descent requires an expedition before the final kill. A golden bullet is hidden along a verified, walkable route about 88–136 metres from the starting point. Its enlarged casing glows fluorescent green even in darkness, with pulsing green rings to make the pickup easy to spot. Follow the map and walk over it to collect it; collection restores up to 20 health and briefly protects the player.
...
The tactical map uses a cyan player arrow, a fluorescent green circle for the bullet and a red circle for Bertranda. A continuous green line with a dark outline shows only the route to the bullet. Its framing includes the whole known route and its detours, even on short phone screens. Collecting gold clears the route and stops route searches; Bertranda remains a red marker with a direct bearing/distance readout. Searches remain bounded when wandering far away; a `~` distance indicates an estimate beyond the local route. Map display and routing do not pause combat or replace the enemy navigation field.
...
A larger fluorescent green arrow at the top of the screen rotates relative to the player's heading and shows the remaining route distance. Its brightness pulses every 1.2 seconds without ever going dark; the reduced-motion preference disables the CSS pulse. It follows nearby reachable waypoints, including turns around walls, instead of pointing straight through obstacles. It works with the minimap closed, disappears on collection, and returns for the next realm. Danger messages temporarily take priority. The phone indicator is 28 pixels tall, with captions beneath it and the reticle, map and controls clear. Guidance reuses the map route; it adds no 3D meshes, lights or extra route searches.
...
The touch interface uses a single 24-pixel status row and a 112-pixel-wide minimap that opens automatically when starting or restarting. Its colour key identifies the cyan player, red Bertranda circle and green bullet circle. The canvas is 66 pixels tall, reduced to 46 on short landscape viewports so it stays above the joystick; MAP can still hide/show it. Small visual icons retain generous touch areas. The central aiming area stays clear. Both desktop and phone use the same marker and route colours.
...
[Bertranda 2](../bertranda-2/) and [the original Bertranda](../bertranda/) remain independent games. The older Roblox place stays with Bertranda 2; this release adds faster browser controls for PC and phone.

## Run locally

Serve the repository with a local HTTP server and open `videogames/bertranda-3/`. Direct `file://` loading is not supported by browser texture security rules.

## Regression checks

Run `node --test videogames/bertranda-3/tests/*.test.cjs` from the repository root. Tests use the bundled Three.js geometry and production game logic with browser I/O mocked. They cover frame-rate-independent walking/sprinting, same-step turning/movement, sensitivity levels, full-distance swipes, collision at the new sprint speed, contextual reminder timing and lifecycle, golden bullet progression, map input, one-thumb aim/fire, pointer cancellation, manual firing, portrait suspension and bounded worlds. Renderer mocks reproduce Three's inline CSS sizing, and rotation tests check display dimensions as well as camera/buffer size.
